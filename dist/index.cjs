"use strict";Object.defineProperty(exports,Symbol.toStringTag,{value:"Module"});const a=require("react/jsx-runtime"),n=require("react"),m=require("esbuild-wasm");function p(t){const e=Object.create(null,{[Symbol.toStringTag]:{value:"Module"}});if(t){for(const r in t)if(r!=="default"){const s=Object.getOwnPropertyDescriptor(t,r);Object.defineProperty(e,r,s.get?s:{enumerable:!0,get:()=>t[r]})}}return e.default=t,Object.freeze(e)}const b=p(m),h=()=>({name:"fetch-plugin",setup(t){t.onResolve({filter:/^(react|react-dom)$/},e=>({path:`https://esm.sh/${e.path}`,namespace:"a"})),t.onResolve({filter:/.*/},e=>({path:`https://esm.sh/${e.path}`,namespace:"a"})),t.onLoad({filter:/.*/},async e=>({contents:await(await fetch(e.path)).text(),loader:"tsx"}))}});let i;const y=async t=>{if(!i)try{i=await b.initialize({wasmURL:"https://unpkg.com/esbuild-wasm@0.19.0/esbuild.wasm",worker:!0})}catch(e){throw console.error("Error initializing esbuild:",e),new Error("Failed to initialize esbuild.")}try{return(await i.build({entryPoints:["index.tsx"],bundle:!0,write:!1,plugins:[h()],stdin:{contents:t,resolveDir:"/",loader:"tsx"}})).outputFiles[0].text}catch(e){throw console.error("Error bundling code:",e),e}},f=()=>{const[t,e]=n.useState(`
...
root.render(<App />);
  `),[r,s]=n.useState(!1),[c,l]=n.useState(null),d=async()=>{s(!0),l(null);try{const o=await y(t),u=document.getElementById("rurybox-preview");u&&(u.srcdoc=`
          <html>
            <head>
              <style>
